...
updated_at: 2026-09-16T19:39:02Z
created_by:
...
- [x] Document a verified authority/permission mapping for all six tools and combined write/network effects
- [x] Show writer ask defaults and network gating remain effective on supported host modes and user overrides remain host-controlled
- [x] Record whether selective Sequential use is needed, with concurrency evidence
- [x] Retain and run SSRF/redirect, path, byte/image/cache limit and output-sanitization regressions; do not claim an SDK trust field that does not exist
- [x] Record host-mode and user-override test cases for both writers and all readers, including overwrite refusal and denied-network behavior; preserve application SSRF defenses even when host permission allows the tool
...
Verified actual v0.137.0 host policy using the real manifest across five modes and user overrides; documented network-read plus writer ask mapping and honest yolo/plan semantics. Concurrent blocked-download regression and all application defense/race tests passed. just ci now includes isolated host-contract tests and passes locally. PR #10 carries the batch; actual host-driver launch follows in conformance.
